...
def main():
    ...
    if args.dataset_name is not None:
        ...
        dataset = load_dataset(
            ...
            data_dir=args.train_data_dir+args.class_name,
        )
    else:
        data_files = {}
        if args.train_data_dir+args.class_name is not None:
            data_files["train"] = os.path.join(args.train_data_dir,args.class_name, "**")
        dataset = load_dataset(
            # "imagefolder",
            'script/'+args.class_name+'/{}_train.py'.format(args.class_name),
    ...
    if args.fault_column is None:
        ...
        if fault_column not in column_names:
            raise ValueError(
                f"--caption_column' value '{args.fault_column}' needs to be one of: {', '.join(column_names)}"
            )
    ...
    if args.mask_column is None:
        ...
        if mask_column not in column_names:
            raise ValueError(
                f"--caption_column' value '{args.mask_column}' needs to be one of: {', '.join(column_names)}"
            )
    ...
    def preprocess_train(examples):
        ...
        for mask in examples[mask_column]:
            ...
            mask[mask >= 0.5] = 1
            masks.append(mask)
            masked_image.append(examples["fault_image"][count] * (mask < 0.5))
            count += 1
    ...
    for epoch in range(first_epoch, args.num_train_epochs+1):
        ...
        if accelerator.is_main_process:
            if args.validation_file is not None and epoch % args.validation_epochs == 0:
                val_examples = []
                with open(os.path.join(args.validation_file), "r") as val_index_fp:
